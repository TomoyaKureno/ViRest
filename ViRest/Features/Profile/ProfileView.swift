...
struct ProfileView: View {
                    ...
                    VStack(spacing: 14) {
                        ...
                        if let info = viewModel.infoMessage {
                            HStack(spacing: 8) {
                                ...
                                Text(info).font(AppTypography.caption(13)).foregroundStyle(.white)
                                Spacer()
                            }
                            .padding(11)
                            .background(
    ...
    // ── Gamification card — uses Firestore totalActionsCompleted as source of truth
    private var gamificationCard: some View {
        let totalSessions = viewModel.firestoreUser?.totalActionsCompleted ?? viewModel.badgeState.completedSessions
        let level = ProgressionLevel.from(completedSessions: totalSessions)

        return SurfaceCard {
            sectionTitle("Progress", icon: "sparkles.rectangle.stack")

            if let title = viewModel.currentTitle {
                row("Title", value: title.name)
            }
            row("Level", value: "Level \(level.rawValue) · \(level.title)")
            row("Completed sessions", value: "\(totalSessions)")
            row("Current streak", value: "\(viewModel.badgeState.currentStreak) days")

            if let nextTarget = level.nextTargetSessions {
                let remaining = max(0, nextTarget - totalSessions)
    ...
    private func sectionTitle(_ title: String, icon: String) -> some View {
        HStack(spacing: 8) {
            ...
            Text(title).font(AppTypography.title(20)).foregroundStyle(AppPalette.textPrimary)
        }
    }

    private func row(_ title: String, value: String) -> some View {
        HStack {
            Text(title).font(AppTypography.caption(13)).foregroundStyle(AppPalette.textSecondary)
            Spacer()
            Text(value).font(AppTypography.body(15)).foregroundStyle(AppPalette.textPrimary)
        }
        .padding(.vertical, 2)
    }
    ...
    private func lockedGoalRow(label: String, value: String) -> some View {
        HStack {
            Text(label).font(AppTypography.caption(13)).foregroundStyle(AppPalette.textSecondary)
            Spacer()
            Text(value).font(AppTypography.body(15)).foregroundStyle(AppPalette.textPrimary)
        }
        .padding(.horizontal, 10)
